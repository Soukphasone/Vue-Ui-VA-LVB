    ...
    <template v-for="(childItem, index) in items" :key="index">
      <li>
        <router-link
          ...
          class="group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-black"
          :class="{
            '!text-black': childItem.label === sidebarStore.selected
          }"
        >
          <!-- {{ childItem.label }} -->
        </router-link>
